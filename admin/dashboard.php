...
// Dashboard with charts, drill, CSV export. CSV/AJAX handlers run before any HTML output.

if (session_status() === PHP_SESSION_NONE) session_start();
ob_start(); // buffer to be safe
...
// ---------------- Read to_be_days from GET/POST/session (no redirect) ----------------
if (isset($_GET['to_be_days'])) {
    $to_be_days = (int)$_GET['to_be_days']; if ($to_be_days < 0) $to_be_days = $DEFAULT_TO_BE_DAYS;
    $_SESSION['to_be_days'] = $to_be_days;
} elseif (isset($_POST['to_be_days'])) {
    $to_be_days = (int)$_POST['to_be_days']; if ($to_be_days < 0) $to_be_days = $DEFAULT_TO_BE_DAYS;
    $_SESSION['to_be_days'] = $to_be_days;
}
$to_be_days = isset($_SESSION['to_be_days']) ? (int)$_SESSION['to_be_days'] : $DEFAULT_TO_BE_DAYS;

// ---------------- AJAX: drill (return JSON) ----------------
    ...
    try {
        if ($type === 'initial') {
            ...
            $sql_page  = "SELECT e.employee_id, e.full_name, e.cost_center, e.position, e.department, t.CompletedDate, t.Status
                          ...
                          ORDER BY e.cost_center, e.employee_id
        ...
        echo json_encode(['ok'=>true,'course'=>['id'=>$course_id,'code'=>$course['course_code'],'name'=>$course['course_name']],'type'=>$type,'to_be_days'=>$to_be,'total'=>$total,'page'=>$page,'page_size'=>$limit,'rows'=>$rows]);
    } catch (Exception $e) {
...
// ---------------- CSV export handler (must run before HTML output) ----------------
if (isset($_GET['export_course'])) {
    $course_id = (int)($_GET['export_course'] ?? 0);
    $type = $_GET['etype'] ?? 'initial'; // initial|expired|tobe
    $to_be = max(0, (int)($_GET['to_be_days'] ?? $to_be_days));
    ...
    $stmt->execute([$course_id]);
    $course = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$course || !safe_table($course['table_name'])) {
        header('HTTP/1.1 400 Bad Request'); echo "Invalid course"; exit;
    }
    $tbl = $course['table_name']; $valid = (int)$course['validity_months'];
    ...
    try {
        ...
        header('HTTP/1.1 500 Internal Server Error'); echo "Query error"; exit;
    }

    // send CSV headers and output only CSV (no HTML)
...
// ---------------- MAIN PAGE: compute aggregated stats (caching) ----------------
...
if ($cached !== null) {
    ...
    $courses = $pdo->query("SELECT course_id, course_code, course_name, COALESCE(validity_months,24) AS validity_months, table_name FROM courses WHERE is_active = 1 AND table_name IS NOT NULL ORDER BY course_code")->fetchAll(PDO::FETCH_ASSOC);
    $total_gap = 0; $course_stats = [];

    foreach ($courses as $c) {
        $cid = (int)$c['course_id']; $code = $c['course_code']; $name = $c['course_name']; $valid = (int)$c['validity_months']; $tbl = $c['table_name'];
        if (!safe_table($tbl)) continue;
        try { $initial = (int)$pdo->query("SELECT COUNT(*) FROM employees e LEFT JOIN `{$tbl}` t ON e.employee_id = t.ID WHERE e.is_active = 1 AND (t.ID IS NULL OR t.CompletedDate IS NULL OR t.CompletedDate = '')")->fetchColumn(); } catch (Exception $e) { $initial = 0; }
        try { $expired = (int)$pdo->query("SELECT COUNT(*) FROM `{$tbl}` t JOIN employees e ON e.employee_id = t.ID WHERE e.is_active = 1 AND t.CompletedDate IS NOT NULL AND DATE_ADD(t.CompletedDate, INTERVAL {$valid} MONTH) < CURDATE()")->fetchColumn(); } catch (Exception $e) { $expired = 0; }
        try { $tobe = (int)$pdo->query("SELECT COUNT(*) FROM `{$tbl}` t JOIN employees e ON e.employee_id = t.ID WHERE e.is_active = 1 AND t.CompletedDate IS NOT NULL AND DATE_ADD(t.CompletedDate, INTERVAL {$valid} MONTH) BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL {$to_be_days} DAY)")->fetchColumn(); } catch (Exception $e) { $tobe = 0; }
        $total = $initial + $expired + $tobe; $course_stats[$cid] = ['course_code'=>$code,'course_name'=>$name,'initial'=>$initial,'expired'=>$expired,'tobe'=>$tobe,'total'=>$total]; $total_gap += $total;
    }

    $dept_gaps = []; $ccs = $pdo->query("SELECT DISTINCT cost_center FROM employees WHERE is_active = 1 ORDER BY cost_center")->fetchAll(PDO::FETCH_COLUMN);
    foreach ($ccs as $cc) {
        $cc_total = 0;
        foreach ($courses as $c) {
            $tbl = $c['table_name']; $valid = (int)$c['validity_months'];
            if (!safe_table($tbl)) continue;
            try { $stmt = $pdo->prepare("SELECT COUNT(*) FROM employees e LEFT JOIN `{$tbl}` t ON e.employee_id = t.ID WHERE e.is_active = 1 AND e.cost_center = :cc AND (t.ID IS NULL OR t.CompletedDate IS NULL OR t.CompletedDate = '')"); $stmt->execute([':cc'=>$cc]); $i = (int)$stmt->fetchColumn(); } catch (Exception $e) { $i = 0; }
            try { $stmt = $pdo->prepare("SELECT COUNT(*) FROM `{$tbl}` t JOIN employees e ON e.employee_id = t.ID WHERE e.is_active = 1 AND e.cost_center = :cc AND t.CompletedDate IS NOT NULL AND DATE_ADD(t.CompletedDate, INTERVAL {$valid} MONTH) < CURDATE()"); $stmt->execute([':cc'=>$cc]); $e = (int)$stmt->fetchColumn(); } catch (Exception $e) { $e = 0; }
            try { $stmt = $pdo->prepare("SELECT COUNT(*) FROM `{$tbl}` t JOIN employees e ON e.employee_id = t.ID WHERE e.is_active = 1 AND e.cost_center = :cc AND t.CompletedDate IS NOT NULL AND DATE_ADD(t.CompletedDate, INTERVAL {$valid} MONTH) BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL {$to_be_days} DAY)"); $stmt->execute([':cc'=>$cc]); $t = (int)$stmt->fetchColumn(); } catch (Exception $e) { $t = 0; }
            $cc_total += ($i+$e+$t);
        }
    ...
    foreach ($courses as $c) {
        ...
        try { $rows = $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC); foreach ($rows as $r) { if (isset($month_counts[$r['ym']])) $month_counts[$r['ym']] += (int)$r['cnt']; } } catch (Exception $e) {}
    }

    $cache_val = ['course_stats'=>$course_stats,'dept_gaps'=>$dept_gaps,'total_gap'=>$total_gap,'month_counts'=>$month_counts,'months'=>$months];
...
$top_dept_values = array_map(function($k) use ($dept_gaps){ return $dept_gaps[$k]; }, $top_dept_labels);
$js_bar_labels = json_encode($top_dept_labels); $js_bar_values = json_encode($top_dept_values);
$js_line_labels = json_encode($months);
$js_line_values = json_encode(array_map(function($m) use ($month_counts){ return $month_counts[$m] ?? 0; }, $months));
...
<div class="max-w-7xl mx-auto p-6">
    ...
    <div class="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="bg-white p-4 rounded-xl shadow">
            ...
            <div class="mt-3 text-sm"><span class="text-gray-600">Click a slice to view employees →</span></div>
        </div>
                    ...
                    <?php foreach ($course_stats as $cid=>$cs): ?>
                        <tr class="border-b">
                            ...
                            <td class="p-2 text-center">
                                <!-- IMPORTANT: link directly to this file for CSV export -->
...
const pie = new Chart(pieCtx, {
    ...
    options: { responsive:true, plugins:{legend:{position:'bottom'}}, onClick(e, items) {
        ...
        const idx = items[0].index; const code = pie.data.labels[idx];
        // map course_code -> course_id via PHP embedded map
        ...
        // open drill in new tab: drill uses AJAX, but for simplicity open page with params
